...
import (
	...
	"github.com/TienMinh25/go-hexagonal-architecture/internal/application/mock"
...
func TestPaymentService_CreatePayment(t *testing.T) {
	...
	paymentType := domainpayment.EWallet
	paymentLogo := gofakeit.ImageURL(320, 320)

	paymentInput := &domainpayment.Payment{
		Name: paymentName,
		Type: paymentType,
		Logo: paymentLogo,
	}
	paymentOutput := &domainpayment.Payment{
		ID:        paymentID,
		Name:      paymentName,
		Type:      paymentType,
		Logo:      paymentLogo,
	...
	for _, tc := range testCases {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			paymentService := NewPaymentUsecase(paymentRepo, cache)
...
func TestPaymentService_ListPayments(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		payments = append(payments, domainpayment.Payment{
			ID:   gofakeit.Uint64(),
			Name: gofakeit.CreditCardType(),
			Type: domainpayment.EWallet,
...
func TestPaymentService_UpdatePayment(t *testing.T) {
	...
	existingPayment := &domainpayment.Payment{
		ID:   paymentID,
		Name: gofakeit.CreditCardType(),
		Type: domainpayment.Cash,
		Logo: gofakeit.ImageURL(320, 320),
	}
